...
@Component({
  ...
  template: `
    <nav>
      <button class="toggle" (click)="show = !show" [disabled]="toggleDisabled">
        Toggle View
      </button>
    </nav>
    <section
      *ngIf="show"
      @query
      (@query.start)="toggleDisabled = true"
      (@query.done)="toggleDisabled = false"
    >
      ...
      <div class="hero">
        ...
        <span class="name"
          >{{ hero.name }} <small>(heroes are always animated!)</small></span
        >
      </div>
    </section>
  `,
